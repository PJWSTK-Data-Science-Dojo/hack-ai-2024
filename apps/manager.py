...
def run_or_die(
    cmd: list[str], error_msg: str = "Error running command."
) -> tuple[bytes, bytes]:
...
class RuntimeEnvironment:
    ...
    def from_args(cls) -> "RuntimeEnvironment":
        ...
        parser.add_argument(
            "-u",
            "--uninstall",
            help="Uninstall the application from the menu.",
            action="store_true",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "app", type=str, help="Path to the application to install/uninstall."
        )
    ...
    def root(self) -> str:
        ...
        stdout, _ = run_or_die(
            ["git", "rev-parse", "--show-toplevel"], "Error finding git repo's root."
        )
...
class App:
    ...
    def __init__(self, path: str) -> None:
        ...
        if not self._path.exists():
            raise RuntimeError(
                f"The specified application does not exist: {str(self._path)}"
            )
    ...
    def config(self) -> dict[str, str]:
        ...
        _, stderr = run_or_die(
            [(self._path), "config"], "Unable to read config from application."
        )
    ...
    def meta(self) -> dict[str, Any]:
        ...
        _, stderr = run_or_die(
            [(self._path), "meta"], "Unable to read metadata from application."
        )
